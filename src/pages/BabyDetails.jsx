      ...
      <div className="flex items-start gap-4 mb-2">
        ...
        <div>
          ...
          <p className="text-sm text-[var(--color-text-muted)] font-medium mt-1">
            Device: <span className="font-mono">{deviceKey}</span> • Assigned: {assignment?.active ? 'Yes' : 'No'}
          </p>
        </div>
      </div>

      <Tabs defaultValue="overview" className="space-y-6">
        <TabsList className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-lg p-1 h-auto">
          <TabsTrigger value="overview" className="py-2.5 px-6 rounded-md font-semibold data-[state=active]:bg-blue-50 data-[state=active]:text-blue-600 dark:data-[state=active]:bg-blue-900/30 dark:data-[state=active]:text-blue-400 shadow-none data-[state=active]:shadow-sm transition-all">Overview</TabsTrigger>
          <TabsTrigger value="history" className="py-2.5 px-6 rounded-md font-semibold data-[state=active]:bg-blue-50 data-[state=active]:text-blue-600 dark:data-[state=active]:bg-blue-900/30 dark:data-[state=active]:text-blue-400 shadow-none data-[state=active]:shadow-sm transition-all">Vitals History</TabsTrigger>
          <TabsTrigger value="alerts" className="py-2.5 px-6 rounded-md font-semibold data-[state=active]:bg-blue-50 data-[state=active]:text-blue-600 dark:data-[state=active]:bg-blue-900/30 dark:data-[state=active]:text-blue-400 shadow-none data-[state=active]:shadow-sm transition-all">
            Alerts <span className="ml-2 bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 rounded-full px-2 py-0.5 text-xs">{alerts.length}</span>
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="overview" className="space-y-6 m-0">
          ...
          {hrStatus === VITAL_STATUS.ALERT && (
            <Alert variant="destructive" className="bg-red-50 border-red-200 text-red-800 dark:bg-red-950/50 dark:border-red-900 dark:text-red-300 shadow-sm animate-in slide-in-from-top-2">
              <HeartPulse className="h-5 w-5 text-red-600 dark:text-red-400 animate-pulse" />
              <AlertTitle className="text-lg font-bold">Critical Heart Rate</AlertTitle>
              <AlertDescription className="font-medium mt-1">Heart rate is currently {currentTelemetry.heartRateBpm} bpm.</AlertDescription>
            </Alert>
          )}

          {spo2Status === VITAL_STATUS.ALERT && (
            <Alert variant="destructive" className="bg-red-50 border-red-200 text-red-800 dark:bg-red-950/50 dark:border-red-900 dark:text-red-300 shadow-sm animate-in slide-in-from-top-2">
              <Beaker className="h-5 w-5 text-red-600 dark:text-red-400 animate-pulse" />
              <AlertTitle className="text-lg font-bold">Critical Oxygen Drop</AlertTitle>
              ...
              <div className="h-64 w-full">
                <ResponsiveContainer>
                  <LineChart data={chartData}>
                    ...
                    />
                    <Line type="stepAfter" dataKey="spo2" stroke="#10B981" strokeWidth={3} dot={false} isAnimationActive={false} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>
                ...
                {alerts.map((alert) => (
                  <div key={alert.id} className="relative">
                    ...
                    <div>
                      ...
                      <p className="text-[var(--color-text-main)] font-medium bg-gray-50 dark:bg-gray-900/50 p-3 rounded-lg border border-[var(--color-border)] inline-block">
                        {alert.reason}
                      </p>
                    </div>
                  </div>
