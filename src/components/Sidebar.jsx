...
import { Activity, LayoutDashboard, MonitorSmartphone, LogOut, Moon, Sun, User } from 'lucide-react';
...
export default function Sidebar() {
  ...
  const handleLogout = () => {
    localStorage.removeItem('mamawatch_auth');
    navigate('/login');
  };
    ...
    <aside className="w-64 bg-[var(--color-surface)] border-r border-[var(--color-border)] flex flex-col h-screen sticky top-0 shrink-0 transition-colors duration-300">
      ...
      <div className="p-4 border-t border-[var(--color-border)]">
        ...
        <div className="flex items-center gap-3 px-2 mb-4">
          ...
          <div>
            <p className="text-sm font-semibold text-[var(--color-text-main)]">Nurse Zainab</p>
            <p className="text-xs text-[var(--color-text-muted)]">NICU Lead</p>
          </div>
        </div>
